onload = function () {
    ...
    function createData(){
        sz = Math.floor(Math.random() * 9) + 6 ;
        const cities = ['Delhi', 'Mumbai', 'Kanpur', 'Jammu', 'Hyderabad', 'Bangalore', 'Gangtok', 'Meghalaya', 'Indore', 'Lucknow', 'Kolkata', 'Rajkot', 'Ranchi', 'Chennai'];
        let nodes = [];
        for(let i=1;i<=sz;i++){
        ...
        src = Math.floor(Math.random()*sz)+1;
        document.getElementById('src_city').innerHTML = cities[src-1];
        dst = src;
        while(src===dst)
        {
            dst = Math.floor(Math.random()*sz)+1;
        }
        document.getElementById('dest_city').innerHTML = cities[dst-1];
    ...
    function solveData(sz) {
        ...
        if(plane !== 0)
        {
            new_edges.push({arrows: { to: { enabled: true}}, from: u+1, to: v+1, color: 'green',label: String(plane)});
            new_edges.concat(pushEdges(dist1, u, false));
            new_edges.concat(pushEdges(dist2, v, true));
        } 
        else
        {
